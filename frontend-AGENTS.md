...
## 主动建议规则
- 发现页面、布局、组件或样式职责混杂，导致文件膨胀、复用困难时，应主动提醒并建议最小拆分点
- 发现样式与 `design-system.yaml`、现有主题或设计规范冲突时，应先说明差异和影响，不直接引入平行视觉体系
- 发现可以复用已有布局、组件、样式变量或交互模式时，应优先建议复用
- 不确定组件行为、布局规则、交互模式或设计 token 时，应按根 `AGENTS.md` 的查证优先级处理，禁止凭经验硬编码
...
## 任务管理机制
`TASK.md` 是唯一活动任务记录文件，只维护当前仍需关注的任务与最近 5 条已完成任务。
...
8. `DONE` 任务超过 5 条时，自动删除最早的 `DONE` 任务。
9. 不再使用 `doc/task-archive/` 归档。
